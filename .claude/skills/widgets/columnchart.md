...
|----------|------|----------|---------|----------------|-------|-------------|
| `series` | object | Yes |  | list; 18 sub-properties below | General::Data Source | Add one or more columns. The order influences how columns overlay one another: the first column (from the top) is drawn lowest and other columns are drawn on top of it. |
| &nbsp;&nbsp;&nbsp;&nbsp;↳ `dataSet` | enumeration | Yes | static | `static` \| `dynamic` |  | Data set |
| &nbsp;&nbsp;&nbsp;&nbsp;↳ `staticDataSource` | datasource |  |  | list |  | Data points for a single column. |
| &nbsp;&nbsp;&nbsp;&nbsp;↳ `dynamicDataSource` | datasource |  |  | list |  | Data points for all columns which will be divided into single columns based on the Group by attribute value. |
| &nbsp;&nbsp;&nbsp;&nbsp;↳ `groupByAttribute` | attribute |  |  |  |  | Data points within the same group form one column. |
| &nbsp;&nbsp;&nbsp;&nbsp;↳ `staticName` | textTemplate |  |  |  |  | The column name displayed in the legend. |
| &nbsp;&nbsp;&nbsp;&nbsp;↳ `dynamicName` | textTemplate |  |  |  |  | The column name displayed in the legend. |
...
| &nbsp;&nbsp;&nbsp;&nbsp;↳ `staticBarColor` | expression |  |  |  |  | Column color |
| &nbsp;&nbsp;&nbsp;&nbsp;↳ `dynamicBarColor` | expression |  |  |  |  | Column color |
...
| `advancedOptions` | boolean | Yes | false |  | General::General | Enable advanced options |
...
| `barmode` | enumeration | Yes | group | `group` \| `stack` | General::General | Column format |
...
Regenerated by `mxcli widget docs` and by `refresh catalog`. For the same data live from the `.mpk` — including anything added by a widget upgrade since this file was written — run `mxcli widget describe columnchart -p <app.mpr>`.
