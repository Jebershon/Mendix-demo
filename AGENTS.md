...
```bash
./mxcli -p LiveLogs.mpr    # Correct - uses local binary
```
...
```bash
./mxcli setup mxbuild -p LiveLogs.mpr    # Auto-download for project's Mendix version
```

After setup, `mx` is at `~/.mxcli/mxbuild/{version}/modeler/mx`. Usage:

```bash
./mxcli docker check -p LiveLogs.mpr   # Validate project (resolves the project's Mendix version)
```

To call `mx` directly, name the version — a `*` glob breaks once two are cached:

```bash
~/.mxcli/mxbuild/<version>/modeler/mx check LiveLogs.mpr
```
...
```bash
./mxcli -p LiveLogs.mpr -c "SHOW MODULES"              # List all modules
./mxcli -p LiveLogs.mpr -c "SHOW STRUCTURE"             # Project overview
./mxcli -p LiveLogs.mpr -c "SHOW ENTITIES IN MyModule"  # Entities in a module
./mxcli -p LiveLogs.mpr -c "DESCRIBE ENTITY MyModule.Customer"  # Entity details
```

### Execute an MDL Script File

```bash
./mxcli exec script.mdl -p LiveLogs.mpr
```
...
```bash
./mxcli
# Then: CONNECT LOCAL 'LiveLogs.mpr';
```
...
./mxcli check script.mdl -p LiveLogs.mpr --references  # With reference validation
```
...
```bash
./mxcli check script.mdl -p LiveLogs.mpr --references
```
...
```bash
# Lint the project
./mxcli lint -p LiveLogs.mpr

# With colored output
./mxcli lint -p LiveLogs.mpr --color

# List available rules
./mxcli lint -p LiveLogs.mpr --list-rules

# Output as SARIF
./mxcli lint -p LiveLogs.mpr --format sarif > results.sarif
```
...
./mxcli report -p LiveLogs.mpr

# JSON report
./mxcli report -p LiveLogs.mpr --format json

# HTML report
./mxcli report -p LiveLogs.mpr --format html
```
